  ...
  methods: {
    ...
    imageTouchMove: function (e) {
      ...
      if (e.touches.length === 1) {
        ...
        // 体验优化，优化双指操作相对于旋转来说太过于灵敏问题
        // 设双指，左指坐标（x1,y1），右指坐标（x2, y2）
        // 在y轴时，需处理双指x轴两点之间的距离，当距离超过阈值时，则启用旋转，判断条件Math.abs(x1 - x2) >= ROTATE_THRESHOLD
        // x轴同理，需处理双指y轴两点之间的距离，当距离超过阈值时，则启用旋转，判断条件Math.abs(y1 - y2) >= ROTATE_THRESHOLD
        const ROTATE_THRESHOLD = 200;
        if (Math.abs(clientXForLeft - clientXForRight) >= ROTATE_THRESHOLD && Math.abs(clientYForLeft - clientYForRight) >= ROTATE_THRESHOLD) {
          if (!this.properties.disableRotate) {
            ...
            this.triggerEvent('linrotate', {
              currentDeg: this.data.angle + current_deg
            }, option);
          }
        }
